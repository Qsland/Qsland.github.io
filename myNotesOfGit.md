...
### 添加远程仓库
1. 在githu创建新的仓库
2. git remote add origin *** 命令，要进入到本地git库的根目录    连接github
   git branch -M main  
   git push -u origin main //origin 是为远程库指定的名字  main也是自己指定的名字

   git remote -v  查看远程库信息
   git remote rm origin 删除远程库origin，即解除与本地库的绑定
3. 第一次推送时用git remote -u origin main 一次性全部把main内的内容提交，后续提交 -u 可以省略
